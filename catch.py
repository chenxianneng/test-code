    ...
    for a_tag in a_tags:
        ...
        try:
            ...
            try:
                ...
                if i > 2 and lastPrice > 1:
                    # 打印股息    
                    averageRate = totalAmount / ((i - 2) / 2) /  (lastPrice * 10)
                    print('平均每10股派发分红：', round(totalAmount / ((i - 2) / 2), 2), '元    平均分红利率：', round(averageRate * 100, 2), '%')
...
# run('http://quote.cfi.cn/stockList.aspx?t=11')
run('http://quote.cfi.cn/stockList.aspx?t=12')
# run('http://quote.cfi.cn/stockList.aspx?t=13')